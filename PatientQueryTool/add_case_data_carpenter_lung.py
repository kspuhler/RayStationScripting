# -*- coding: utf-8 -*-
"""
Created on Fri Jan 23 15:45:33 2026
...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 23 15:27:45 2026
...
from Planning.Structures.Functions.checkForContour import checkForContour, checkForContourSubstring
    ...
    for idx, record in enumerate(records):
        ...
        for c in cases:
            if any(x.lower() in c.CaseName.lower() for x in ["DNU", "TEST"]):
                continue
            c.SetCurrent()
            plan_names = [x.Name for x in c.TreatmentPlans]
            if not c.BodySite:
                #if checkForContour("PTV4500") or checkForContour("MDGTV") or checkForContour("Bladder") or checkForContour("Rectum"): #SUCCESS
                #    c.BodySite = "Pelvis"
                if any("lung" in x.lower() for x in plan_names) or checkForContour("ITV") or checkForContourSubstring("bronc") or checkForContourSubstring("distal") or checkForContourSubstring("prox"):
