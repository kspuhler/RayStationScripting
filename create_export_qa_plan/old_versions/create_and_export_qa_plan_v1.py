...
def set_ui_to_current_qa_beamset(beam_set):
    # Save patient to update UI
    save_patient()
    ...
    infos = plan.QueryBeamSetInfo(Filter = {'Name': beam_set.DicomPlanLabel})
...
def ask_user_for_shifts(beam_set):
    
    # Get verification plan
    verification_plans = get_verification_plans(beam_set)
    ...
    for verification_plan in verification_plans:
        ...
        while ask_for_shifts:
            ...
            if verification_plan.BeamSet.FractionDose.DoseValues is not None: #Exit flag of while loop
                ...
                calc_dose(verification_plan.BeamSet)
...
def dose_and_plan_export_to_file(beam_set):
    ...
    # Create export directory for plan and dose if it does not exist
    path = create_qa_path()
    ...
    try:
        for verification_plan in verification_plans:
            ...
            result = verification_plan.ScriptableQADicomExport(ExportFolderPath = path,
        ...
        log_warning(error)
        user_message('Plan and dose export to file failed.  Check execution details for more info.')
        raise error

def create_qa_plan(beam_set):
    ...
    if len(verification_plans) == 0:
        ...
        # Set gantry angle to zero for the 21EX only
        if machine == '21EX':
            gantry_angle = 0
        else:
            gantry_angle = None
        ...
        # Set QA plan name
        qa_plan_name = beam_set.DicomPlanLabel + "_QA" 
        qa_plan_name = qa_plan_name[:16]
...
def export_qa_to_raygateway(beam_set):
    ...
    for verification_plan in verification_plans:
        try:
            ...
            log_warning(error)
            user_message('Export to iDMS failed.  The treatment plan must be exported to iDMS prior to a QA plan export.  Check execution details for more info.')
            raise error

def create_qa_path():

    # Define your path and directory
    patient = get_current("Patient")
    path = r"\\Client\F$\SHARING\Radiation Oncology Physics\IMRT QA"
    directory = patient.PatientID + "_" + patient.Name
...
def message_create_qa_plan_yes_no(beam_set):
    ...
    response = messagebox.askyesno(
        ...
        f"Do you want to create a QA plan for {bs.DicomPlanLabel} even though it has a delivery technique of {bs.DeliveryTechnique}?"
    )
...
def get_verification_plans(beam_set):
    ...
    plan = get_current("Plan")
    verification_plans = [v for v in plan.VerificationPlans if v.OfRadiationSet.DicomPlanLabel == beam_set.DicomPlanLabel]

    return verification_plans

def user_no_verification_plan():
    set_progress(f'User selected not to create a QA plan on {bs.DicomPlanLabel} beam set...')
    sleep(3)
