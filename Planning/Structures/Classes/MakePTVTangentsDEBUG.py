...
class MakePTVTangents(MakePTV):
    ...
    PTV Tangent DEBUGs will be the result of cropping the 50% IDL into the External contour, reducing 6mm, and cropping out of Lung/Liver/Heart.
    ...
    def makePtv(self):
        ...
        if not checkForContour('PTV Tangents'):
            self.pm.CreateRoi(Name="PTV Tangent DEBUGs", Color="Red", Type="ptv", TissueName="", RbeCellTypeName=None, RoiMaterial=None)
        ...
        #Crop into External
        self.pm.RegionsOfInterest['PTV Tangents'].CreateAlgebraGeometry(Examination=self.exam, Algorithm="Auto", ExpressionA={ 'Operation': "Union", 'SourceRoiNames': ["PTV Tangent DEBUGs"], 'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 } }, 
        ...
        #Crop 0.6cm 
        self.pm.RegionsOfInterest['PTV Tangents'].CreateAlgebraGeometry(Examination=self.exam, Algorithm="Auto", ExpressionA={ 'Operation': "Union", 'SourceRoiNames': ["PTV Tangent DEBUGs"], 'MarginSettings': { 'Type': "Contract", 'Superior': 0.6, 'Inferior': 0.6, 'Anterior': 0.6, 'Posterior': 0.6, 'Right': 0.6, 'Left': 0.6} },
        ...
        if tb is not None:
            ...
            self.pm.CreateRoi(Name="PTV_TB_EValDEBUG", Color="Pink", Type="ptv", TissueName="", RbeCellTypeName=None, RoiMaterial=None)
            self.pm.RegionsOfInterest['PTV TB Eval'].CreateAlgebraGeometry(Examination=self.exam, Algorithm="Auto", ExpressionA={ 'Operation': "Union", 'SourceRoiNames': [tb], 'MarginSettings': { 'Type': "Expand", 'Superior': 1.0, 'Inferior': 1.0, 'Anterior': 1.0, 'Posterior': 1.0, 'Right': 1.0, 'Left': 1.0 } },
                                                                            ExpressionB={ 'Operation': "Union", 'SourceRoiNames': [], 'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 } }, ResultOperation="None", 
                                                                            ResultMarginSettings={ 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 })
            
            self.pm.RegionsOfInterest['PTV TB Eval'].CreateAlgebraGeometry(Examination=self.exam, Algorithm="Auto", ExpressionA={ 'Operation': "Union", 'SourceRoiNames': ["PTV_TB_EValDEBUG"], 'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 } }, 
    ...
    def checkForDoseCalc(self):
        ...
        if self.plan.TreatmentCourse.TotalDose.GetDoseStatistic(RoiName=self.externalName, DoseType="Max") > 0:
            ...
            tmp.SetMUAndComputeDose(ForceRecompute=True)
    ...
    def __repr__(self):
        return  '''This script makes PTV Tangent DEBUGs by calculating plan to give rx dose to norm point and then taking 50%idl and cropping it from anything with "lung" "heart" or "liver" in the name \n
