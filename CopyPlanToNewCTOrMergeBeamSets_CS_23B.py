...
Copy Plan to New CT or Merge Beamsets 3.0.20
...
if __name__ == '__main__':
    ...
    if dialog.DialogResult:
        ...
        if dialog.mode == 'copy':
            ...
            if version >= 11:
                ...
                print(reg.all())
                print(reg)
                if  type(reg) == 'NoneType' or reg.all() == None:
                    print ('********************')
    ...
    for bs in beamSets:
        ...
        # not an electron, is it protons?
            ...
            thisObjective = originalPlan.PlanOptimizations[bsNumber].Objective
            if thisObjective and originalPlan.PlanOptimizations[bsNumber].Objective.ConstituentFunctions.Count > 0:
        ...
        # must be a VMAT
            ...
            for (i, beam) in enumerate(bs.Beams):
                ...
                for (j, s) in enumerate(beam.Segments):
                    rpbs.Beams[i].Segments[j].LeafPositions = s.LeafPositions
                    rpbs.Beams[i].Segments[j].JawPositions = s.JawPositions
                    rpbs.Beams[i].Segments[j].DoseRate = s.DoseRate
                    rpbs.Beams[i].Segments[j].RelativeWeight = s.RelativeWeight
