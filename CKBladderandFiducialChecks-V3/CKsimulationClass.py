...
LOG_PATH = "\\\Client\F$\SHARING\Radiation Oncology Physics\Raystation Fiducial Check"
...
class CKsimulation():
    ...
    def __init__(self, *args):
        ...
        self.dis_threshold = 1.8 #fiducial spacing threshold in cm
        ...
        if (self.Contours['Fiducial-Markers'].HasContours() == True):
            ...
            for i in range(0, 4):
                ...
                if name in self.fiducials_list:
                 print('From insertFiducialMarkers: Fiducial: ', name, ' already exists!')
    ...
    def count_fiducials(self, *args):
        ...
        if marker_count == 0:
            msg2 = 'There seem to be no fiducials identified. '
            print(msg2)
    ...
    def getFiducialCoordinates(self, *args):
        ...
        try:
            for i, j in enumerate(self.Fiducials):
                ...
                marker_type = j.OfPoi.Type
                print(i, '\t', pt_name, '\t', marker_type)
                
                if marker_type == 'Marker' and 'Fid' in pt_name:
    ...
    def minAngleTest(self, *args):
        ...
        return a Pass (Boolean True). Otherwise, the test fails (Boolean False). The app will also test that
        all the fiducials are spaced gt. 1cm apart
        """
        fail_angle_count = 0
        num_fids               = self.count_fiducials()
        num_possible_triangles = m.comb(num_fids, 3)
        num_seg                = len(self.pair_distance)
        pair_lt                = self.pair_distance[self.pair_distance < self.dis_threshold]
        num_lt_thresh          = len(pair_lt)
        any_less_than          = np.any(self.pair_distance < self.dis_threshold)
        at_least_three         = num_seg-num_lt_thresh
        ...
        print("ANY LESS THAN:",any_less_than)
        print("NUM OF POSSIBLE TRIANGLES:", num_possible_triangles)
        print("NUM OF SEGMENTS:", num_seg)
        print("NUMBER OF SEGS LT 1.8cm:",num_lt_thresh)
        print("Number of possible triangles:", num_possible_triangles)
        print("ARE THERE AT LEAST 3 SEGMENTS GT 18mm?:",at_least_three)
        
        if num_possible_triangles <= fail_angle_count or at_least_three<3:
...
def make_contour_list(self, *args):
    ...
    for i in self.Contours:
        if (i.HasContours() == True): #List containing only structures with geometries
         names = i.OfRoi.Name
         self.contour_list.append(names)
    for i in self.Contours:   #List of all structures
         names = i.OfRoi.Name
...
def passFail_log(self,result):
    "Make a log file of the fiducials passing or failing the min angle test"
    "1: Pass, 0: Fail"
    pID = self.patient.PatientID 
    case = self.case.CaseName
    exam = self.exam_name
   
    now = dt.datetime.now()
    t= now.timetuple()
    ts = str(t[0])+str(t[1])+str(t[2])+str(t[3])+str(t[4])+str(t[5])+str(t[6])+str(t[7])
    
    fs = pID+'_'+case+'_'+exam+ts+'.log'
    print('Logfile path:', LOG_PATH)
    print('File:',fs)
    file_path = os.path.join(LOG_PATH, fs)
