...
DIR_PATH = "\\\Client\F$\SHARING\Radiation Oncology Physics\RaystationScriptingPROD\CKBladderandFiducialChecks-V3"
...
class BladderTestGUI(QWidget,CKsimulation):
    ...
    def __init__(self, *args):   
        ...
        self.ok    = os.path.join(DIR_PATH, 'check3.png')
        self.notok = os.path.join(DIR_PATH, 'checkNG.png')
        self.warn  = os.path.join(DIR_PATH, 'exclimation.png')
        
        self.play = os.path.join(DIR_PATH, 'play2.jpg')
        self.info = os.path.join(DIR_PATH, 'info.png')
...
class FiducialTestGUI(QWidget,CKsimulation):
    ...
    def __init__(self, *args):   
        ...
        self.play   =  os.path.join(DIR_PATH, 'play2.jpg')
        self.manual =  os.path.join(DIR_PATH, 'hand-color-icon.png')
        self.info   =  os.path.join(DIR_PATH, 'info.png')
        ...
        self.playButton.setIcon(self.icon)
        self.button_bar.addWidget(self.playButton)
        self.playButton.clicked.connect(self.find_and_place_auto)
        self.playButton.setToolTip('Place the fiducial markers automatically')
        
        self.manualButton = QPushButton("")
        self.manualButton.setDisabled(True)
        self.icon = QIcon(self.manual)
        self.manualButton.setIcon(self.icon)
        self.button_bar.addWidget(self.manualButton)
        self.manualButton.clicked.connect(self.find_and_place)
        self.manualButton.setToolTip('Place the fiducial markers manually')
    ...
    def find_and_place_auto(self, *args):
        """
        Locate high-Z markers in a volume and contour them.
        
        Use scikit-learn clustering algorithm to find the centroids of the N
        fiducials, insert N-marker POIs then assign the centroid coordinates
        to each of the POIs.
        """
        ...
        try:
         ctName    = self.examination.Name
         seeds_con = self.case.PatientModel.StructureSets[ctName].RoiGeometries['Fiducial-Markers'].PrimaryShape.Contours
         
         my_con = []
         
         # add the seed points to a list
         for i in range(len(seeds_con)):
           slice_con = seeds_con[i]
           for j in range(len(slice_con)):
             my_con.append([slice_con[j].x, slice_con[j].y, slice_con[j].z])
         
         # convert the list to a ndarray for input into DBSCAN
         data = np.array(my_con, dtype='float32') 
         
         db = DBSCAN(eps=0.35, min_samples=5).fit(data)
         labels = db.labels_
         
         label_set = set(labels)
         label_set.discard(-1) #-1 label is a designated as noise. We discard these if they exist
         #print(label_set)
         
         num_clus = len(label_set) 
         for k in range(num_clus):
          points_of_cluster = data[labels==k,:]
          centroid_of_cluster = np.mean(points_of_cluster, axis=0) 
          print("Centroid of cluster: " + str(centroid_of_cluster)) 
          name = 'Fid' + str(k + 1)
          
          if name in self.fiducials_list:
                   print('From find_and_place_auto: Fiducial: ', name, ' already exists!')
                   self.case.PatientModel.StructureSets[0].PoiGeometries[name].Point={'x':centroid_of_cluster[0],
                                                                                      'y':centroid_of_cluster[1] ,
                                                                                      'z':centroid_of_cluster[2]}
          else:
            self.case.PatientModel.CreatePoi(Examination=self.examination,
                                                   Point={'x':centroid_of_cluster[0],
                                                          'y':centroid_of_cluster[1] ,
                                                          'z':centroid_of_cluster[2]} ,
    ...
    def helpMe(self, *args):
        ...
        msg=""" 
                ...
                The requirement is that there exists at least one triangle formed from the fiducials whose angles are greater than 15 degrees and whose segment lengths (fiducial spacings) are greater than 18mm
                ...
                This app will list all the constituent angles of the a N possible triangles and their values in degrees as well as the spacing bewtween the fiducials. 
                
                If this value is less than 20mm, they will be flagged as possibly problematic for tracking purposes.
...
def setFiducialSpacingInfo(self, *args):
    ...
    for i in range(0, num_pairs):
        ...
        elif distance<self.dis_threshold:
          faillabel = QLabel()
          faillabel.setText("")
          faillabel.setScaledContents(True)
          pixmap = QPixmap()
          pixmap.load(self.notok,'.png')
          faillabel.setPixmap(pixmap)
          self.fidspace.setCellWidget(i,2,faillabel) 
        else:
          warninglabel = QLabel()
