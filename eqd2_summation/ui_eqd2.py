...
class EQD2SelectionWindow(QWidget):
    ...
    def _build_flat_plan_data(self) -> dict[str, dict]:
        ...
        for plan_name, beamsets in nested.items():
            for beamset_name, info in (beamsets or {}).items():
                if self.pdu.has_dose_on_plan_beamset(plan_name, beamset_name):
        ...
        if not flat:
            exc = RuntimeError("No plans with dose.")
            self._prompt_user_acknowledge(exc=exc, message="No plans with dose found.  Script aborting.")
            self._abort_script_error(exc=exc)
    ...
    def _get_user_selections(self) -> dict[dict[str, Any]]:
        ...
        _logger.debug("User selected CT name: %s", ct_exam_name)
        if not ct_exam_name:
        ...
        try:
            ...
            try: 
                for row in range(self.tbl_sel_fraction_doses.rowCount()):
                    ...
                    plan_data_fractions_rs = self.plan_data[dose_name]["fractions"] or None
                    plan_data_plan_name = self.plan_data[dose_name]["plan_name"] or None
                    plan_data_beamset_name = self.plan_data[dose_name]["beamset_name"] or None
                    plan_data_beamset = self.plan_data[dose_name]["beamset"] or None
                    plan_data_ct_exam_name = self.plan_data[dose_name]["ct_exam_name"] or None
                    plan_data_ct_exam = self.plan_data[dose_name]["ct_object"] or None
                    
                    if not plan_data_plan_name or not plan_data_beamset_name:
                        errors.append(f"{plan_data_plan_name}_{plan_data_beamset_name} is empty.  Select a new plan's beamset.")
                    if not dose_obj:
                        errors.append(f"{plan_data_plan_name}_{plan_data_beamset_name} has no dose.  Select a new plan's beamset.")
                    if not nfx:
                        errors.append(f"{plan_data_plan_name}_{plan_data_beamset_name} has no fractions.  Select a new plan's beamset.")
                    if not plan_data_fractions_rs:
                        errors.append(f"{plan_data_plan_name}_{plan_data_beamset_name} has no fractions in Raystation.  Select a new plan's beamset.")
                    if not plan_data_ct_exam_name or not plan_data_ct_exam:
                        errors.append(f"{plan_data_plan_name}_{plan_data_beamset_name} has no CT exam.  Select a new plan's beamset.")
                
                    plan_data[dose_name] = { 
                        "dose": dose_obj, 
                        "fractions": nfx,
                        "fractions_rs": plan_data_fractions_rs,
                        "plan_name": plan_data_plan_name,
                        "beamset_name": plan_data_beamset_name,
                        "beamset": plan_data_beamset,
                        "ct_exam_name": plan_data_ct_exam_name,
                        "ct_exam": plan_data_ct_exam
                        }
            except Exception as e:
                _logger.error("Failed to get data from UI.")
                self._prompt_user_acknowledge(
                    exc=e, 
                    message="Failed to get data from UI.  Script will abort."
                )
